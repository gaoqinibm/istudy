...
### 数据集成与ETL概念
    数据源识别：确定所有需要整合到数据仓库中的数据源。
    数据抽取：从各个数据源抽取数据。
    数据清洗与转换：清洗数据，处理缺失值、异常值，并进行必要的格式转换。
    数据加载：将清洗后的数据加载到数据仓库中。

    数据清洗解决:
    去重：删除完全重复的记录。
    缺失值处理：填充或补全关键字段的缺失信息。
    错误修正：修正明显错误的值，如将"男"、"0"统一为"男性"。

    数据标准化解决：
    格式统一：将日期、电话号码、地址等都统一成一种标准格式。
    字段拆分：例如，将"某某省某某市某某区"这样的复合地址，拆分成"省"、"市"、"区"独立的字段，便于后续分析。
...
### 大数据治理的需求
    ...
    数据治理本质:去重、处理空值、异常值、格式不一致
    数据治理最核心的三件事：把脏数据变干净（清洗）、给数据贴标签（标注）、搞清楚数据来龙去脉（血缘）。
