## AI三件套：数据、算法、算力
    AI的三件套，包括：数据、算法及算力。
    数据是 “生产原料”，提供 AI 学习的基础信息；
    算法是 “生产工艺”，决定加工数据的步骤，以及以产生智能的决策；
    算力是 “生产设备”，支撑算法高效地处理海量数据
![img.png](AI三件套.png)
...
## Dify和Ollama区别
    Dify 是一款开源的大语言模型(LLM) 应用开发平台。它融合了后端即服务和 LLMOps(大语言模型运维) 的理念，使开发者可以快速搭建生产级的生成式 AI 应用。
...
## RAG 生成阶段设置温度值(temperature)的作用
    温度值控制 LLM 输出文本的随机性和创造性，它作用于模型从概率分布中采样 token 的过程。
    t=0.0   → 确定性输出，相同输入总是得到相同结果
    t=0.3   → 低随机性，主要选高概率 token
    t=0.7   → 中等随机性，有一些词汇变化
    t=1.0   → 高随机性，出现意料之外的词汇组合
    t=1.5+  → 极高随机性，可能输出混乱或无意义内容
    
    低（0.0-0.3）	严格依赖检索内容，不发挥
    中（0.5-0.7）	在事实基础上自然表达
    高（>0.8）	可能脱离检索内容“发挥”
    
    控制幻觉风险
    低温度（≤0.3）：模型倾向于选择高概率、更“安全”的 token，不会冒险生成不确定内容
    高温度（≥0.7）：低概率 token 被选中的概率增加，可能出现：
    编造不存在的药品名称
    给出错误的用药剂量
    产生与检索知识库矛盾的回答
    
    温度 0.0 - 每次回答几乎完全相同
    温度控制 LLM 的“冒险程度”——在医疗这种高风险场景，不要让模型冒险，使用 0.1-0.3 的低温度值。
...
![img.png](7大核心Agent全景图.png)
![img.png](系统架构图.png)

## 医疗 AI Agent 六层架构
![img.png](医疗Agent 六层架构.png)

## 项目回顾
### AI 医疗影像辅助诊断平台架构
    医疗影像辅助诊断系统实现，涵盖 DICOM 影像接入、多模态大模型结节识别、结构化报告生成三个核心环节。
    影像接入层：通过 dcm4che 对接医院 PACS 系统，解析 CT/MRI 的 DICOM 文件，提取影像元数据和像素数据，转换为大模型可识别的 PNG 格式
    AI 分析层：基于 Spring AI 的 ChatClient 多模态能力，将影像图像与诊断提示词一起发送给大模型，识别癌症结节
    报告输出层：通过 BeanOutputConverter 将大模型输出转换为结构化 Java 对象，生成 JSON 格式诊断报告
    
    医院 PACS:PACS就是医院的“影像云相册”，拍完的CT、X光、核磁这些片子，直接存进电脑系统里，医生随时调出来看。