...
## Agent + 数仓
    Agent 的引入并不是要替代这些人，而是在人和数仓之间插入一层"智能代理层"，把高频、重复、规则明确的操作自动化掉。
![img.png](Agent 驱动智能数仓的分层设计.png)

    Agent驱动的智能数仓总体架构，五层分层设计，从交互层到数据基础设施层，Agent编排层是核心枢纽整个架构分为五个层次，各层职责划分如下：
    交互层：面向用户，提供自然语言查询、智能看板、对话界面、告警控制台等入口。用户不需要写 SQL，直接用业务语言表达需求即可。
    Agent 编排层：这是整个架构的核心枢纽，包含多个专项 Agent——NL2SQL Agent 负责自然语言转查询，数据质量 Agent 负责异常检测与修复，血缘分析 Agent 负责数据影响评估，ETL 编排 Agent 负责任务自动化生成，告警根因 Agent 负责故障诊断。
    智能体核心引擎：提供所有 Agent 共享的基础能力，包括大语言模型调用、提示词管理、工具注册中心、记忆与上下文管理、规划与推理模块。
    工具与服务层：封装了与数仓基础设施交互的具体能力，SQL 生成器与校验器、元数据查询 API、数据探查引擎、调度引擎、通知服务、日志分析器等。
    数据基础设施层：底层是大家熟悉的数仓技术栈，包括各类数据源（MySQL、Oracle、Kafka、S3）、数据集成工具（Airflow、DolphinScheduler）、数仓引擎（Hive、ClickHouse、Doris）、数据湖（Iceberg、Hudi）、元数据中心（DataHub、Atlas）以及监控体系（Prometheus、Grafana）。

    可落地场景:
    1.NL2SQL Agent让业务方自助查数
