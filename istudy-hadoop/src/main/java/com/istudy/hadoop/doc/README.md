...
## 数据质量管理
    数据质量评估
    1. 完整性(完整性检测:数据记录和必填字段列表)
    2. 准确性(格式检测（正则表达式）,范围检测（数值/日期范围）,枚举值检测,依赖字段检测（如：下单时间 < 付款时间）)
    3. 及时性(检测数据的时效性，包括延迟、更新频率、时间间隔,如单条/批量数据延迟检测(record 数据记录,timeField 时间字段名,maxDelaySeconds 最大允许延迟秒数))
    4. 一致性(跨表字段一致性检测采用INNER JOIN组装SQL语句(table1,table2,joinKey,checkFields)计算总对比数,使用MD5校验整条记录是否一致(对比源库和目标库的哈希值),)
...
### HDFS架构
    HDFS是业界默认的大数据存储系统，在业界的大数据集群中有非常广泛的使用。HDFS集群有着很高的稳定性且易扩展得益于它较简单的架构，
    但包含几千个节点，保存上百拍比特（PB）数据的集群也不鲜见。如图所示HDFS的架构
...
## 新一代元数据管理平台-OpenMetadata
    OpenMetadata一站式元数据管理平台
    OpenMetadata 是一款功能强大的开源元数据管理平台，旨在帮助企业更好地发现、理解和管理其数据资产。
    它提供了一套全面的工具和功能，涵盖了数据发现、数据血缘、数据质量、数据探查、数据治理和团队协作等多个方面。
    是一款功能强大、灵活且易于使用的元数据管理平台，能够帮助企业更好地管理和利用其数据资产。
...
## 数据血缘逻辑
    记录一个数据从哪里来（来源）、经过怎样的处理（过程）、最终去到哪里（去向）
    生产级方案：集成 Apache Atlas 这样的专业元数据治理平台。它能自动构建血缘关系并提供可视化图谱，适合大数据场景。
    轻量级方案：自行设计数据库表来记录关系，通过SQL解析器（如 JSqlParser）或手动埋点来捕获信息。适合中小型项目或特定业务
