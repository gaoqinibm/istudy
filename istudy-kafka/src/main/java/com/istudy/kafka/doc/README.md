...
### 分区计算公式
    根据业务期望的目标吞吐量和压测得到的单分区吞吐量，用这个公式来确定分区数：
    分区数 = 目标吞吐量 / 单分区最大吞吐量

    单分区最大吞吐量 有两个口径：生产者写入和消费者读取。实际系统中，瓶颈往往是处理能力较弱的那一方。所以更严谨的公式是：
    Num = max( T / P, T / C )
    T (Target Throughput)：业务预期的目标吞吐量（如 100 MB/s 或 100,000 条/秒）。
    P (Producer Throughput)：单个分区的写入吞吐量（需压测）。
    C (Consumer Throughput)：单个分区的消费吞吐量（取决于你的业务逻辑，这通常是主要瓶颈）。
    简单说，分区数需要同时满足生产和消费的需求，取两者中的较大值。
    Kafka 支持动态增加分区（使用 kafka-topics.sh --alter 命令），但无法减少，所以初始值宁少勿多(可以增加，但不能减少)
